function drawVerticalDendrogram(e) {
			...
			var baseSvg = d3.select("#tree-container").append("svg").attr(
					"width", viewerWidth).attr("height", viewerHeight).attr(
			...
			function centerNode(source) {
				...
				d3.select('g').transition().duration(duration).attr(
						"transform",
			...
			//	oneLevel = true;
				...
				nodeEnter.append("circle").attr('class', 'nodeCircle').attr(
						"r", 0).style("fill", function(d) {
	...
	/*			if (document.getElementById('attributes').checked)
				...
				if (document.getElementById('objects').checked)
					node.append("text").attr("transform", "translate(15, 12)")
							.text(function(d) {
				...
				if (document.getElementById('objectCount').checked)
					node.append("text").attr("transform", "translate(15, 24)")
							.text(function(d) {
...
*/
			...
			var oneLevel = true;//$('input[name=type]:checked', '#expanded').val() == "oneLevel";
